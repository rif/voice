...
<xsl:template match="sect1                      |/section                      |section[local-name(parent::*) != 'section']" mode="calculate.chunks">
  <xsl:choose>
    ...
    <xsl:when test="$chunk.first.sections = 0">
      <xsl:if test="count(preceding-sibling::section) &gt; 0                     or count(preceding-sibling::sect1) &gt; 0">
        <xsl:call-template name="chunk.info"/>
      </xsl:if>
    </xsl:when>
    <xsl:otherwise>
...
&gt;
      ...
      <chunks xmlns="http://www.w3.org/1999/xhtml">
